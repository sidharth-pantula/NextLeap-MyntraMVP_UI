import { Product } from '../types';

export const MOCK_PRODUCTS: Product[] = [
  {
    id: 'prod-01',
    brand: 'Manyavar Mohey',
    title: 'Dusty Rose Embroidered Silk Lehenga Set',
    description: 'A stunning dusty rose silk lehenga set with intricate floral silver and zari embroidery. Paired with a matching embroidered blouse and soft net dupatta.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 8999,
    originalPrice: 14999,
    discountPercentage: 40,
    rating: 4.8,
    ratingCount: 520,
    images: [
      'https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Festive', 'Party'],
      styles: ['Elegant', 'Statement'],
      look: 'Elegant',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Regular',
      color: 'Dusty Rose',
      colorFamily: 'Pastel',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 3,
      pattern: 'Embroidered',
      tags: ['lehenga', 'wedding', 'silk', 'embroidery', 'pastel', 'ethnic', 'bridal', 'reception']
    },
    signals: {
      viewsCount: 14,
      addedDaysAgo: 3,
      inCart: true,
      priceDropAmount: 1500,
      priceDropPercentage: 14,
      popularityScore: 96,
      stockCount: 2
    }
  },
  {
    id: 'prod-02',
    brand: 'Sera',
    title: 'Crimson Solid Satin Cowl-Neck Maxi Evening Gown',
    description: 'Deep crimson red evening gown crafted in luxurious satin with a fluid drape, cowl neckline and subtle back crisscross straps.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 2499,
    originalPrice: 5999,
    discountPercentage: 58,
    rating: 4.3,
    ratingCount: 1240,
    images: [
      'https://images.unsplash.com/photo-1566174053879-31528523f8ae?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Party', 'Cocktail', 'Wedding'],
      styles: ['Statement', 'Trendy'],
      look: 'Statement',
      productType: 'Western',
      fabric: 'Satin',
      fit: 'Slim',
      color: 'Crimson Red',
      colorFamily: 'Dark',
      formality: 'Semi-Formal',
      isFlashy: true,
      durabilityRating: 3,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Solid',
      tags: ['gown', 'dress', 'satin', 'cocktail', 'evening', 'party', 'red']
    },
    signals: {
      viewsCount: 19,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 500,
      priceDropPercentage: 17,
      popularityScore: 92,
      stockCount: 5
    }
  },
  {
    id: 'prod-03',
    brand: 'Biba',
    title: 'Emerald Velvet Flared Evening Gown Kurta Set',
    description: 'Regal emerald green maxi flared gown tailored in premium velvet with fine crystal work along the neckline and cuffs.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 7450,
    originalPrice: 10500,
    discountPercentage: 29,
    rating: 4.7,
    ratingCount: 380,
    images: [
      'https://images.unsplash.com/photo-1539008835657-9e8e9680c956?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Cocktail', 'Party', 'Festive'],
      styles: ['Elegant', 'Classic'],
      look: 'Elegant',
      productType: 'Ethnic',
      fabric: 'Velvet',
      fit: 'Regular',
      color: 'Emerald Green',
      colorFamily: 'Dark',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 3,
      pattern: 'Solid',
      tags: ['gown', 'velvet', 'emerald', 'wedding', 'reception', 'ethnic']
    },
    signals: {
      viewsCount: 8,
      addedDaysAgo: 7,
      inCart: false,
      popularityScore: 88,
      stockCount: 4
    }
  },
  {
    id: 'prod-04',
    brand: 'Kalki Fashion',
    title: 'Peach Pearl Embellished Georgette Saree',
    description: 'Ethereal pastel peach georgette saree featuring delicate handcrafted pearl borders and an embroidered silk unstitched blouse piece.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 9200,
    originalPrice: 12000,
    discountPercentage: 23,
    rating: 4.9,
    ratingCount: 610,
    images: [
      'https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['Free Size'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Festive'],
      styles: ['Elegant', 'Classic'],
      look: 'Elegant',
      productType: 'Ethnic',
      fabric: 'Georgette',
      fit: 'Regular',
      color: 'Pastel Peach',
      colorFamily: 'Pastel',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 3,
      pattern: 'Embroidered',
      tags: ['saree', 'pastel', 'wedding', 'pearl', 'georgette', 'ethnic', 'reception']
    },
    signals: {
      viewsCount: 12,
      addedDaysAgo: 5,
      inCart: false,
      priceDropAmount: 800,
      priceDropPercentage: 8,
      popularityScore: 94,
      stockCount: 3
    }
  },
  {
    id: 'prod-05',
    brand: 'Ritu Kumar',
    title: 'Ivory Silk Zari Anarkali Kurta Suit Set',
    description: 'Classically tailored ivory silk anarkali suit featuring gold zari bootis, a churidar and a contrasting banarasi dupatta.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 6800,
    originalPrice: 9990,
    discountPercentage: 32,
    rating: 4.5,
    ratingCount: 420,
    images: [
      'https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Festive', 'Everyday'],
      styles: ['Minimal', 'Classic'],
      look: 'Minimal',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Tailored',
      color: 'Ivory',
      colorFamily: 'Neutral',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Embroidered',
      tags: ['anarkali', 'silk', 'zari', 'ivory', 'wedding', 'daywear', 'ethnic']
    },
    signals: {
      viewsCount: 7,
      addedDaysAgo: 9,
      inCart: false,
      popularityScore: 86,
      stockCount: 6
    }
  },
  {
    id: 'prod-06',
    brand: 'MANGO',
    title: 'Emerald Silk Flutter-Sleeve Wrap Maxi Dress',
    description: 'Sophisticated emerald green silk wrap dress with short flutter sleeves, a tie-up waist and an elegant asymmetrical ruffled hem.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 4799,
    originalPrice: 7999,
    discountPercentage: 40,
    rating: 4.6,
    ratingCount: 890,
    images: [
      'https://images.unsplash.com/photo-1572804013309-59a88b7e92f1?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Cocktail', 'Party', 'Wedding', 'Vacation'],
      styles: ['Elegant', 'Minimal'],
      look: 'Elegant',
      productType: 'Western',
      fabric: 'Silk',
      fit: 'Regular',
      color: 'Emerald',
      colorFamily: 'Dark',
      formality: 'Semi-Formal',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['dress', 'maxi', 'silk', 'wrap dress', 'emerald', 'wedding guest']
    },
    signals: {
      viewsCount: 15,
      addedDaysAgo: 2,
      inCart: true,
      priceDropAmount: 1200,
      priceDropPercentage: 20,
      popularityScore: 93,
      stockCount: 3
    }
  },
  {
    id: 'prod-07',
    brand: 'Fabindia',
    title: 'Handcrafted Indigo Pure Cotton Tiered Maxi Dress',
    description: 'Artisanal hand block printed pure cotton tiered midi dress with mandarin collar and shell button detailing.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 1899,
    originalPrice: 3299,
    discountPercentage: 42,
    rating: 4.6,
    ratingCount: 1420,
    images: [
      'https://images.unsplash.com/photo-1515372039744-b8f02a3ae446?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Vacation', 'Everyday', 'Work'],
      styles: ['Casual', 'Minimal'],
      look: 'Casual',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Relaxed',
      color: 'Indigo Blue',
      colorFamily: 'Dark',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Printed',
      tags: ['cotton', 'casual', 'vacation', 'everyday', 'summer', 'breathable', 'dress']
    },
    signals: {
      viewsCount: 22,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 400,
      priceDropPercentage: 17,
      popularityScore: 95,
      stockCount: 8
    }
  },
  {
    id: 'prod-08',
    brand: 'Zara',
    title: 'Structured Double-Breasted Linen Blazer',
    description: 'Crisp lightweight pure linen tailored blazer with structured shoulders, horn buttons and welt pockets.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 3590,
    originalPrice: 5990,
    discountPercentage: 40,
    rating: 4.7,
    ratingCount: 740,
    images: [
      'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Casual', 'Everyday'],
      styles: ['Minimal', 'Elegant'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Linen',
      fit: 'Tailored',
      color: 'Oatmeal Beige',
      colorFamily: 'Neutral',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['blazer', 'linen', 'workwear', 'minimal', 'smart casual', 'office', 'jacket']
    },
    signals: {
      viewsCount: 11,
      addedDaysAgo: 4,
      inCart: false,
      popularityScore: 90,
      stockCount: 4
    }
  },
  {
    id: 'prod-09',
    brand: 'Marks & Spencer',
    title: 'Pure Irish Linen Regular Fit Formal Shirt',
    description: 'Breathable pure Irish linen solid formal shirt with classic collar, single-cuff sleeves and curved hem.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 2299,
    originalPrice: 3499,
    discountPercentage: 34,
    rating: 4.6,
    ratingCount: 940,
    images: [
      'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['39', '40', '42', '44'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Casual', 'Everyday', 'Vacation'],
      styles: ['Minimal', 'Classic'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Linen',
      fit: 'Regular',
      color: 'Optic White',
      colorFamily: 'Neutral',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['shirt', 'linen', 'formal', 'office', 'work', 'summer', 'men']
    },
    signals: {
      viewsCount: 16,
      addedDaysAgo: 2,
      inCart: true,
      priceDropAmount: 400,
      priceDropPercentage: 15,
      popularityScore: 91,
      stockCount: 6
    }
  },
  {
    id: 'prod-10',
    brand: 'H&M',
    title: 'Trendy Oversized Satin Camp Collar Shirt',
    description: 'Statement drop-shoulder camp collar shirt cut in glossy fluid satin with abstract modern micro prints.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 1499,
    originalPrice: 2499,
    discountPercentage: 40,
    rating: 4.4,
    ratingCount: 630,
    images: [
      'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Party', 'Vacation', 'Casual'],
      styles: ['Trendy', 'Statement'],
      look: 'Trendy',
      productType: 'Western',
      fabric: 'Satin',
      fit: 'Oversized',
      color: 'Monochrome Black & Silver',
      colorFamily: 'Dark',
      formality: 'Casual',
      isFlashy: true,
      durabilityRating: 3,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Printed',
      tags: ['shirt', 'satin', 'oversized', 'trendy', 'party', 'clubwear']
    },
    signals: {
      viewsCount: 18,
      addedDaysAgo: 1,
      inCart: false,
      priceDropAmount: 300,
      priceDropPercentage: 17,
      popularityScore: 89,
      stockCount: 5
    }
  },
  {
    id: 'prod-11',
    brand: 'Louis Philippe',
    title: 'Men Charcoal Slim Fit Tailored Formal Trousers',
    description: 'Wrinkle-resistant poly-viscose charcoal formal trousers with a clean flat front and tailored stretch waistband.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 2599,
    originalPrice: 3999,
    discountPercentage: 35,
    rating: 4.5,
    ratingCount: 1120,
    images: [
      'https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['30', '32', '34', '36'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Formal', 'Everyday'],
      styles: ['Minimal', 'Classic'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Poly Viscose',
      fit: 'Slim',
      color: 'Charcoal Grey',
      colorFamily: 'Dark',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['trousers', 'pants', 'formal', 'work wear', 'office', 'charcoal', 'men']
    },
    signals: {
      viewsCount: 9,
      addedDaysAgo: 8,
      inCart: false,
      popularityScore: 87,
      stockCount: 7
    }
  },
  {
    id: 'prod-12',
    brand: 'W for Woman',
    title: 'Mustard Georgette Floral Flared Kurta Set',
    description: 'Contemporary asymmetrical A-line georgette kurta with palazzos and botanical digital prints.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 2199,
    originalPrice: 4599,
    discountPercentage: 52,
    rating: 4.4,
    ratingCount: 980,
    images: [
      'https://images.unsplash.com/photo-1609357605129-26f69add5d6e?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Work', 'Festive', 'Everyday'],
      styles: ['Trendy', 'Casual'],
      look: 'Trendy',
      productType: 'Ethnic',
      fabric: 'Georgette',
      fit: 'Regular',
      color: 'Mustard Yellow',
      colorFamily: 'Bright',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Floral',
      tags: ['kurta', 'fusion', 'georgette', 'floral', 'summer', 'daywear', 'ethnic']
    },
    signals: {
      viewsCount: 14,
      addedDaysAgo: 3,
      inCart: true,
      priceDropAmount: 600,
      priceDropPercentage: 21,
      popularityScore: 91,
      stockCount: 4
    }
  },
  {
    id: 'prod-13',
    brand: 'Vero Moda',
    title: 'Pastel Mint Green Satin Midi Slip Dress',
    description: 'Effortlessly chic bias-cut satin slip dress with spaghetti straps and fluid drape for evening parties.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 2799,
    originalPrice: 4999,
    discountPercentage: 44,
    rating: 4.6,
    ratingCount: 840,
    images: [
      'https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Party', 'Cocktail', 'Vacation'],
      styles: ['Trendy', 'Minimal'],
      look: 'Trendy',
      productType: 'Western',
      fabric: 'Satin',
      fit: 'Slim',
      color: 'Mint Green',
      colorFamily: 'Pastel',
      formality: 'Semi-Formal',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 4,
      pattern: 'Solid',
      tags: ['dress', 'slip dress', 'satin', 'pastel', 'mint', 'party']
    },
    signals: {
      viewsCount: 20,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 600,
      priceDropPercentage: 18,
      popularityScore: 94,
      stockCount: 4
    }
  },
  {
    id: 'prod-14',
    brand: 'AND',
    title: 'Classic Wide-Leg Pleated Tailored Trousers',
    description: 'High-waisted wide-leg tailored trousers in lightweight crepe fabric with side slant pockets.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 1999,
    originalPrice: 3499,
    discountPercentage: 43,
    rating: 4.5,
    ratingCount: 650,
    images: [
      'https://images.unsplash.com/photo-1509551388413-e18d0ac5d495?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['28', '30', '32', '34'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Casual', 'Everyday'],
      styles: ['Minimal', 'Elegant'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Poly Viscose',
      fit: 'Relaxed',
      color: 'Beige Camel',
      colorFamily: 'Neutral',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['trousers', 'pants', 'wide leg', 'workwear', 'office', 'beige']
    },
    signals: {
      viewsCount: 16,
      addedDaysAgo: 3,
      inCart: false,
      priceDropAmount: 500,
      priceDropPercentage: 20,
      popularityScore: 90,
      stockCount: 5
    }
  },
  {
    id: 'prod-15',
    brand: 'Levi\'s',
    title: '511 Slim Fit Stretch Denim Jeans',
    description: 'Iconic medium indigo wash 511 slim fit jeans with performance stretch comfort and classic 5-pocket styling.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 2699,
    originalPrice: 4299,
    discountPercentage: 37,
    rating: 4.7,
    ratingCount: 3200,
    images: [
      'https://images.unsplash.com/photo-1542272604-780c96856592?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['30', '32', '34', '36'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Everyday', 'Vacation'],
      styles: ['Casual', 'Classic'],
      look: 'Casual',
      productType: 'Western',
      fabric: 'Denim',
      fit: 'Slim',
      color: 'Medium Indigo',
      colorFamily: 'Dark',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['jeans', 'denim', 'pants', 'levis', 'casual', 'everyday', 'stretch']
    },
    signals: {
      viewsCount: 25,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 500,
      priceDropPercentage: 16,
      popularityScore: 98,
      stockCount: 12
    }
  },
  {
    id: 'prod-16',
    brand: 'Rareism',
    title: 'Minimal Lilac Cotton High-Low Blouse Top',
    description: 'Modern relaxed silhouette top tailored in 100% fine organic cotton with spread collar and pleated back.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 1599,
    originalPrice: 2999,
    discountPercentage: 47,
    rating: 4.5,
    ratingCount: 310,
    images: [
      'https://images.unsplash.com/photo-1534126511673-b6899657816a?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Work', 'Everyday'],
      styles: ['Minimal', 'Trendy'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Relaxed',
      color: 'Lilac Pastel',
      colorFamily: 'Pastel',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['top', 'shirt', 'blouse', 'cotton', 'pastel', 'workwear', 'summer']
    },
    signals: {
      viewsCount: 13,
      addedDaysAgo: 3,
      inCart: false,
      priceDropAmount: 400,
      priceDropPercentage: 20,
      popularityScore: 89,
      stockCount: 5
    }
  },
  {
    id: 'prod-17',
    brand: 'Kalyan Silks',
    title: 'Kanjeevaram Gold & Crimson Pure Silk Saree',
    description: 'Authentic handwoven pure mulberry silk Kanjeevaram saree featuring grand korvai gold zari border and rich pallu.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 14500,
    originalPrice: 22000,
    discountPercentage: 34,
    rating: 4.9,
    ratingCount: 780,
    images: [
      'https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['Free Size'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Festive'],
      styles: ['Statement', 'Classic'],
      look: 'Statement',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Regular',
      color: 'Crimson & Gold',
      colorFamily: 'Bright',
      formality: 'Ultra Formal',
      isFlashy: true,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 3,
      pattern: 'Embroidered',
      tags: ['kanjeevaram', 'silk', 'saree', 'wedding', 'bridal', 'gold zari']
    },
    signals: {
      viewsCount: 28,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 2500,
      priceDropPercentage: 15,
      popularityScore: 99,
      stockCount: 1
    }
  },
  {
    id: 'prod-18',
    brand: 'U.S. Polo Assn.',
    title: 'Tailored Fit Solid Pique Cotton Polo Shirt',
    description: 'Signature combed cotton pique polo shirt with ribbed collar, two-button placket and brand embroidery.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 1299,
    originalPrice: 2199,
    discountPercentage: 41,
    rating: 4.5,
    ratingCount: 2100,
    images: [
      'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Everyday', 'Vacation'],
      styles: ['Casual', 'Classic'],
      look: 'Casual',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Regular',
      color: 'Navy Blue',
      colorFamily: 'Dark',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['polo', 'tshirt', 'shirt', 'cotton', 'casual', 'everyday', 'summer']
    },
    signals: {
      viewsCount: 17,
      addedDaysAgo: 2,
      inCart: false,
      priceDropAmount: 300,
      priceDropPercentage: 19,
      popularityScore: 92,
      stockCount: 9
    }
  },
  {
    id: 'prod-19',
    brand: 'House of Pataudi',
    title: 'Regal Black Zari Embroidered Silk Blend Kurta Set',
    description: 'Exquisite jet black royal kurta crafted with self-motif embroidery along collar and placket, paired with churidar trousers.',
    category: 'Men',
    subCategory: 'Ethnic Wear',
    price: 3499,
    originalPrice: 6999,
    discountPercentage: 50,
    rating: 4.7,
    ratingCount: 890,
    images: [
      'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['38', '40', '42', '44'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Party', 'Festive'],
      styles: ['Elegant', 'Statement'],
      look: 'Elegant',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Tailored',
      color: 'Jet Black',
      colorFamily: 'Dark',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Embroidered',
      tags: ['kurta', 'men ethnic', 'wedding', 'pataudi', 'silk', 'reception']
    },
    signals: {
      viewsCount: 21,
      addedDaysAgo: 1,
      inCart: true,
      priceDropAmount: 700,
      priceDropPercentage: 17,
      popularityScore: 97,
      stockCount: 3
    }
  },
  {
    id: 'prod-20',
    brand: 'Forever New',
    title: 'Champagne Satin Pleated Halter Neck Party Dress',
    description: 'Glamorous champagne satin pleated cocktail dress with halter neckline, cinched waist, and flowy midi skirt.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 4999,
    originalPrice: 8499,
    discountPercentage: 41,
    rating: 4.7,
    ratingCount: 520,
    images: [
      'https://images.unsplash.com/photo-1566174053879-31528523f8ae?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Party', 'Cocktail', 'Wedding'],
      styles: ['Elegant', 'Statement'],
      look: 'Statement',
      productType: 'Western',
      fabric: 'Satin',
      fit: 'Regular',
      color: 'Champagne Gold',
      colorFamily: 'Metallic',
      formality: 'Semi-Formal',
      isFlashy: true,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 4,
      pattern: 'Solid',
      tags: ['dress', 'party dress', 'satin', 'cocktail', 'metallic', 'evening']
    },
    signals: {
      viewsCount: 19,
      addedDaysAgo: 2,
      inCart: true,
      priceDropAmount: 1100,
      priceDropPercentage: 18,
      popularityScore: 95,
      stockCount: 3
    }
  },
  {
    id: 'prod-21',
    brand: 'Libas',
    title: 'Floral Printed Pure Cotton Flared Anarkali Kurta Set',
    description: 'Vibrant turquoise pure cotton flared anarkali kurta paired with matching printed trousers and lightweight dupatta.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 1899,
    originalPrice: 4299,
    discountPercentage: 55,
    rating: 4.6,
    ratingCount: 1650,
    images: [
      'https://images.unsplash.com/photo-1609357605129-26f69add5d6e?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Festive', 'Everyday', 'Work'],
      styles: ['Trendy', 'Casual'],
      look: 'Trendy',
      productType: 'Ethnic',
      fabric: 'Cotton',
      fit: 'Regular',
      color: 'Turquoise Blue',
      colorFamily: 'Bright',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Floral',
      tags: ['kurta', 'anarkali', 'cotton', 'floral', 'ethnic', 'libas']
    },
    signals: {
      viewsCount: 22,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 93,
      stockCount: 8
    }
  },
  {
    id: 'prod-22',
    brand: 'Jack & Jones',
    title: 'Slim Fit Acid Wash Trucker Denim Jacket',
    description: 'Rugged vintage faded blue trucker denim jacket featuring buttoned chest flap pockets and adjustable waist tabs.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 2799,
    originalPrice: 4999,
    discountPercentage: 44,
    rating: 4.7,
    ratingCount: 820,
    images: [
      'https://images.unsplash.com/photo-1576995853123-5a10305d93c0?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Everyday', 'Vacation'],
      styles: ['Casual', 'Trendy'],
      look: 'Casual',
      productType: 'Western',
      fabric: 'Denim',
      fit: 'Slim',
      color: 'Vintage Light Blue',
      colorFamily: 'Pastel',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['jacket', 'denim', 'trucker', 'casual', 'streetwear', 'men']
    },
    signals: {
      viewsCount: 18,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 91,
      stockCount: 6
    }
  },
  {
    id: 'prod-23',
    brand: 'AND',
    title: 'Pleated Accordion Shimmer A-Line Midi Skirt',
    description: 'Sunburst pleated metallic finish A-line midi skirt with elasticized shimmer waistband and fluid drape.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 2199,
    originalPrice: 3999,
    discountPercentage: 45,
    rating: 4.5,
    ratingCount: 430,
    images: [
      'https://images.unsplash.com/photo-1583496661160-fb5886a0aaaa?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['28', '30', '32', '34'],
    inStock: true,
    attributes: {
      occasions: ['Party', 'Work', 'Casual'],
      styles: ['Trendy', 'Statement'],
      look: 'Trendy',
      productType: 'Western',
      fabric: 'Poly Viscose',
      fit: 'Regular',
      color: 'Bronze Metallic',
      colorFamily: 'Metallic',
      formality: 'Semi-Formal',
      isFlashy: true,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 4,
      pattern: 'Solid',
      tags: ['skirt', 'pleated', 'midi skirt', 'party', 'metallic', 'and']
    },
    signals: {
      viewsCount: 15,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 88,
      stockCount: 5
    }
  },
  {
    id: 'prod-24',
    brand: 'Fabindia',
    title: 'Handblock Printed Pure Tussar Silk Kurta',
    description: 'Rich mustard yellow tussar silk kurta detailed with subtle geometric hand-block patterns and classic mandarin collar.',
    category: 'Men',
    subCategory: 'Ethnic Wear',
    price: 2999,
    originalPrice: 4999,
    discountPercentage: 40,
    rating: 4.8,
    ratingCount: 910,
    images: [
      'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['38', '40', '42', '44'],
    inStock: true,
    attributes: {
      occasions: ['Festive', 'Wedding', 'Party'],
      styles: ['Elegant', 'Classic'],
      look: 'Elegant',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Regular',
      color: 'Mustard Yellow',
      colorFamily: 'Bright',
      formality: 'Ultra Formal',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Printed',
      tags: ['kurta', 'tussar silk', 'ethnic', 'men', 'festive', 'fabindia']
    },
    signals: {
      viewsCount: 26,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 94,
      stockCount: 4
    }
  },
  {
    id: 'prod-25',
    brand: 'H&M',
    title: 'Relaxed Fit Chunky Ribbed Knit Cardigan',
    description: 'Oversized V-neck button-down cardigan knitted in ultra-soft blend with tortoise-shell buttons and drop shoulders.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 2299,
    originalPrice: 3999,
    discountPercentage: 42,
    rating: 4.6,
    ratingCount: 1120,
    images: [
      'https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Everyday', 'Work'],
      styles: ['Minimal', 'Casual'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Relaxed',
      color: 'Oatmeal Beige',
      colorFamily: 'Neutral',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['cardigan', 'knitwear', 'winter', 'sweater', 'minimal', 'casual']
    },
    signals: {
      viewsCount: 20,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 90,
      stockCount: 7
    }
  },
  {
    id: 'prod-26',
    brand: 'Tommy Hilfiger',
    title: 'Striped Oxford Pure Cotton Casual Shirt',
    description: 'Classic preppy vertical stripe button-down shirt in 100% breathable Oxford cotton with signature chest flag embroidery.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 3499,
    originalPrice: 5999,
    discountPercentage: 41,
    rating: 4.7,
    ratingCount: 1350,
    images: [
      'https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['39', '40', '42', '44'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Casual', 'Everyday'],
      styles: ['Minimal', 'Classic'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Regular',
      color: 'Sky Blue Stripe',
      colorFamily: 'Pastel',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Striped',
      tags: ['shirt', 'oxford', 'striped', 'cotton', 'work', 'casual', 'men']
    },
    signals: {
      viewsCount: 17,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 92,
      stockCount: 5
    }
  },
  {
    id: 'prod-27',
    brand: 'Global Desi',
    title: 'Boho Print Tiered Georgette Maxi Fusion Dress',
    description: 'Flowy sleeveless tiered maxi dress in vibrant indigo geometric prints with drawstring tassel waist belt.',
    category: 'Women',
    subCategory: 'Ethnic Wear',
    price: 2199,
    originalPrice: 3999,
    discountPercentage: 45,
    rating: 4.5,
    ratingCount: 680,
    images: [
      'https://images.unsplash.com/photo-1572804013309-59a88b7e92f1?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['XS', 'S', 'M', 'L'],
    inStock: true,
    attributes: {
      occasions: ['Vacation', 'Festive', 'Casual'],
      styles: ['Trendy', 'Casual'],
      look: 'Trendy',
      productType: 'Ethnic',
      fabric: 'Georgette',
      fit: 'Relaxed',
      color: 'Navy & Rust',
      colorFamily: 'Dark',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 4,
      pattern: 'Printed',
      tags: ['maxi dress', 'fusion', 'boho', 'georgette', 'ethnic', 'vacation']
    },
    signals: {
      viewsCount: 19,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 89,
      stockCount: 6
    }
  },
  {
    id: 'prod-28',
    brand: 'Snitch',
    title: 'Cuban Collar Textured Waffle Knitted Polo',
    description: 'Trendy retro-inspired short-sleeve resort polo crafted in waffle knit breathable blend with open Cuban collar.',
    category: 'Men',
    subCategory: 'Western Wear',
    price: 1499,
    originalPrice: 2499,
    discountPercentage: 40,
    rating: 4.4,
    ratingCount: 940,
    images: [
      'https://images.unsplash.com/photo-1618354691373-d851c5c3a990?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['S', 'M', 'L', 'XL'],
    inStock: true,
    attributes: {
      occasions: ['Casual', 'Party', 'Vacation'],
      styles: ['Trendy', 'Casual'],
      look: 'Trendy',
      productType: 'Western',
      fabric: 'Cotton',
      fit: 'Relaxed',
      color: 'Sage Green',
      colorFamily: 'Pastel',
      formality: 'Casual',
      isFlashy: false,
      durabilityRating: 4,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['polo', 'knit', 'cuban collar', 'resort wear', 'casual', 'trendy']
    },
    signals: {
      viewsCount: 23,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 91,
      stockCount: 8
    }
  },
  {
    id: 'prod-29',
    brand: 'Zara',
    title: 'Tailored High-Waist Wide Leg Linen Trousers',
    description: 'Chic high-rise straight wide-leg trousers cut in lightweight pure linen-blend with deep side pockets and pressed pleats.',
    category: 'Women',
    subCategory: 'Western Wear',
    price: 2990,
    originalPrice: 4990,
    discountPercentage: 40,
    rating: 4.7,
    ratingCount: 810,
    images: [
      'https://images.unsplash.com/photo-1509551388413-e18d0ac5d495?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['28', '30', '32', '34'],
    inStock: true,
    attributes: {
      occasions: ['Work', 'Casual', 'Vacation'],
      styles: ['Minimal', 'Elegant'],
      look: 'Minimal',
      productType: 'Western',
      fabric: 'Linen',
      fit: 'Relaxed',
      color: 'Natural Ecru',
      colorFamily: 'Neutral',
      formality: 'Smart Casual',
      isFlashy: false,
      durabilityRating: 5,
      comfortRating: 5,
      versatilityRating: 5,
      pattern: 'Solid',
      tags: ['trousers', 'linen', 'wide leg', 'workwear', 'summer', 'minimal']
    },
    signals: {
      viewsCount: 21,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 93,
      stockCount: 4
    }
  },
  {
    id: 'prod-30',
    brand: 'Manyavar',
    title: 'Jacquard Woven Nehru Jacket with Silk Kurta Set',
    description: 'Royal blue woven jacquard sleeveless bundi/Nehru jacket layered over an ivory raw silk kurta pajama set.',
    category: 'Men',
    subCategory: 'Ethnic Wear',
    price: 5999,
    originalPrice: 9999,
    discountPercentage: 40,
    rating: 4.9,
    ratingCount: 1190,
    images: [
      'https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?w=800&auto=format&fit=crop&q=80'
    ],
    sizes: ['38', '40', '42', '44'],
    inStock: true,
    attributes: {
      occasions: ['Wedding', 'Festive', 'Party'],
      styles: ['Statement', 'Elegant'],
      look: 'Statement',
      productType: 'Ethnic',
      fabric: 'Silk',
      fit: 'Tailored',
      color: 'Royal Blue & Ivory',
      colorFamily: 'Dark',
      formality: 'Ultra Formal',
      isFlashy: true,
      durabilityRating: 5,
      comfortRating: 4,
      versatilityRating: 4,
      pattern: 'Embroidered',
      tags: ['nehru jacket', 'bundi', 'kurta set', 'wedding', 'manyavar', 'ethnic']
    },
    signals: {
      viewsCount: 28,
      addedDaysAgo: 0,
      inCart: false,
      popularityScore: 96,
      stockCount: 3
    }
  }
];

export const INITIAL_WISHLIST_IDS = [
  'prod-01', 'prod-02', 'prod-03', 'prod-04', 'prod-05',
  'prod-06', 'prod-07', 'prod-08', 'prod-09', 'prod-10',
  'prod-11', 'prod-12', 'prod-13', 'prod-14', 'prod-15',
  'prod-16', 'prod-17', 'prod-18', 'prod-19', 'prod-20'
];
